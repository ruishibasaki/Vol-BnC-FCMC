...
public:
   ...
   inline void cc(const double gamma, const VOL_dvector& w, int actvSSz) {
	 ...
	 abort();
      ...
      const double one_gamma = 1.0 - gamma;
       //std::cout<<actvSSz<<", "<<sz<<" "<<w.sz<<std::endl;
       for (int i = 0; i<actvSSz;++i ){
           //std::cout<<actvS[i]<<std::endl;
           v[i] = one_gamma*(v[i]) + gamma*w[i];
       }
       if(actvSSz<0){
...
public:
   ...
   double ascent(const VOL_dvector& v, const VOL_dvector& last_u, int actvSSz) const;
...
public:
   ...
   virtual int compute_rc(const VOL_dvector& u, VOL_dvector& rc, int actvSSz) = 0;

  
   virtual int solve_subproblem(const VOL_dvector& xstar,
   ...
   virtual int resolve_subproblem(const VOL_dvector& dual, VOL_dvector& rc,
    ...
    virtual int addVI(int iter,double lcost, const VOL_dvector& xstar,
          const VOL_dvector& x, VOL_dvector& dstar,  VOL_dvector& dualu, VOL_dvector& dual_lb, VOL_dvector& dual_ub,
          VOL_dvector& rc, VOL_dvector& h, int & actvSSz)=0;
...
public:
   ...
   int psize;        
   /** length of total dual solution (INPUT) */
   int dsize;
