...
MCND_lp::select_branching_candidates(const BCP_lp_result& lpres,
	...
	if(current_level() == 0 && current_iteration()<20 && !no_heur) return BCP_DoNotBranch;
	/*if(current_level() == 0){
        return BCP_DoNotBranch_Fathomed;
	}*/
...
MCND_lp::logical_fixing(const BCP_lp_result& lpres,
	...
	if(lpchecker.solved) {
 	...
 	if((gap<0.01) && !getOsiVolBabSolver()->has_checked && (!reduced_run && times_dived<3 /*1000 for the biggest inst*/)){
